...
public class Wifi_HiddenAp_Service extends Service {
    private String tag = "Wifi_HiddenAp_Service";
    private int Loop_cnt = HiddenApConnectActivity.Loop_cnt;
    ...
    public void onCreate() {
        Log.i(tag, HiddenApConnectActivity.Loop_cnt + "WifiStart.");
        super.onCreate();
	...
	public Handler mHandler = new Handler() {
        public void handleMessage(Message msg) {
            ...
            Loop_thread = new Thread(new Runnable() {
                public void run() {
                    Loop_cnt = HiddenApConnectActivity.Loop_cnt;
...
//                            }
                        ...
                        if (ConnectResult && DisconnectResult) {
                            Intent intent = new Intent(HiddenApConnectActivity.WIFI_CONNECT_SUCCESS);
                            sendBroadcast(intent);
                        } else {
                            Intent intent = new Intent(HiddenApConnectActivity.WIFI_CONNECT_FAIL);
                            sendBroadcast(intent);
                        }
                        i++;
                    }
                    Log.d(tag, "Send WIFI_CONNECT_FINISH");
                    Intent intent = new Intent(HiddenApConnectActivity.WIFI_CONNECT_FINISH);
                    sendBroadcast(intent);
                }
            });
            Loop_thread.start();
    ...
    class DefaultBroadcastReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (WifiManager.SUPPLICANT_STATE_CHANGED_ACTION.equals(action)) {
                ...
                WifiInfo wi = Wifi_HiddenAp_Service.mWifiManager.getConnectionInfo();
